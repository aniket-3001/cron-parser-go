 ...
 * Field value ranges.
 *
 * The original builds these as recursive literal unions so that an
 * out-of-range value is a compile error. That construction is reproduced here
 * because the original's type tests assert on assignability; the port itself
 * validates at construction time instead, since Go cannot express a bounded
 * integer.
 */
