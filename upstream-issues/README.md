...
Seven were filed upstream on 2026-08-01. **Five have been fixed and merged.** Of the two still
open, one duplicates a pull request that was already open, and the other carries a pull request
of ours that the maintainer invited, see [Outcome](#outcome) below for the honest accounting.

| Filed | Bug | Severity | Status |
|---|---|---|---|
| [#424](https://github.com/harrisiirak/cron-parser/issues/424) | `stringify()` round trip changes the schedule | **High**: silently reschedules a job | **fixed, [PR #434](https://github.com/harrisiirak/cron-parser/pull/434)** |
| [#419](https://github.com/harrisiirak/cron-parser/issues/419) | DST compensation assumes one-hour transitions | High: an occurrence is skipped in `Antarctica/Troll` | **[PR #435](https://github.com/harrisiirak/cron-parser/pull/435) open**, invited by the maintainer |
...
| [#423](https://github.com/harrisiirak/cron-parser/issues/423) | A duplicated `0` escapes validation, masks later duplicates, and breaks `stringify()` | Medium: accepted input cannot be rendered | labelled `bug`, assigned, and a **duplicate of [PR #418](https://github.com/harrisiirak/cron-parser/pull/418)** |
...
| [PR #434](https://github.com/harrisiirak/cron-parser/pull/434) → #424 | A day field covering its whole range is kept out of the wildcard form, so rendering no longer turns a daily schedule into a monthly one. |
...
**Triaged but not yet fixed.** #423 is labelled and assigned to the maintainer, and duplicates
[PR #418](https://github.com/harrisiirak/cron-parser/pull/418).

**Offered, then opened.** #419 was the only one left untriaged. A working patch existed, offset-based gap
...
pull request. The maintainer replied "sure, open a PR, I'll have a look", so it is now open as
[PR #435](https://github.com/harrisiirak/cron-parser/pull/435), rebased onto current `master` and
re-verified there: 303 of 303 tests pass, the three new regression tests fail without the change,
and the sweep still moves only the 23 `Antarctica/Troll` cases.
...
**So the defensible claim is six original findings, of which five are merged**, not seven
discoveries.

## How they were found

#419, #420, #421 and #422 came from reading the source and confirming by execution. #423, #424 and
#425 came from checking a property over randomly generated expressions: that an expression and
its rendering describe the same schedule. None of the three would have been found by reading.
...
#423 began as two findings. They share one root cause and one fix, because a field that survives
the duplicate check is the same field that cannot be rendered, so they were merged before filing
